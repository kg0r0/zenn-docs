---
title: "SimpleWebAuthn を使用した WebAuthn の Relying Party の実装の勘所 (TypeScript)"
emoji: "🔑"
...
今回は Server Requirements and Transport Binding Profile[^4] に合わせる形で実装例を用意してみたため、こちらをもとに説明をします。
...
それでは、今回は基本的な WebAuthn RP の実装について触れたので、次回は流行の Passkeys を使用する場合を想定した実装に触れてみようと思います。
ではまた！
